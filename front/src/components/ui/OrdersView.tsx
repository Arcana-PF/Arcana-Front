...
const OrdersView = () => {
  ...
  const loadOrders = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setError(typeof result.error === "string" ? result.error : "Error al obtener tus órdenes");
      }
    } catch (err: unknown) {
      setError(err instanceof Error ? err.message : "Error inesperado al cargar órdenes");
    } finally {
      setLoading(false);
    }
  }, [userData?.token]);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-purple-900 via-black to-gray-900 flex items-center justify-center text-gray-200 relative">
  ...
  return (
    <div className="bg-gradient-to-br from-purple-900 via-black to-gray-900 min-h-screen py-10 px-4 text-gray-300 relative font-serif">
      <div className="absolute inset-0 pointer-events-none overflow-hidden">
        <div className="absolute top-1/4 left-1/3 w-64 h-64 bg-yellow-500/20 rounded-full blur-3xl animate-pulse" />
        <div className="absolute bottom-1/4 right-1/3 w-96 h-96 bg-purple-500/20 rounded-full blur-3xl animate-pulse delay-1000" />
      </div>
      ...
      <div className="max-w-3xl mx-auto bg-black/80 backdrop-blur-md rounded-xl border border-yellow-500/30 shadow-xl p-6 relative z-10">
        ...
        {orders.length ? (
          ...
          <div className="text-center italic text-yellow-500">
